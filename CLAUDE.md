...
- **`src/lib/ios-layouts.ts` — where each iPhone browser keeps Add to Home Screen, per iOS version.** Safari 26+ (Compact layout: **•••** by the address bar → Share), Safari 15–25 and ≤14 (centre Share button), Chrome (Share icon at the end of the address bar; needs iOS 16.4, older is sent to Safari), Firefox/Edge (≡ → Share). Each row has `verifiedOn`/`verifiedBy`; `MAX_VERIFIED_IOS` is the newest iOS anyone has checked it on and `ios-layouts.test.ts` fails if it's bumped without a row reaching it. **The September ritual:** after each iOS release, open the app on it in Safari and Chrome, check the buttons, add/extend rows, bump the number. Preview any variant on desktop with `?platform=ios-safari&ios=17.5` (forced views aren't logged).
