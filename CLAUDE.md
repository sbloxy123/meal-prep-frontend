...
- **Credits, trial and plan — `menu.allowance` (`AiAllowance` in `menu.tsx`).** Derived from `GET /shopping-list`'s `entitlement` (backend `lib/credits.js` shape): `plan` (`free` | `trial` | `premium`), `isPremium` (premium OR trial), `trialDaysLeft`, `used/limit/remaining` (credits this period; `Infinity` when unlimited), `resetsAt` (the signup-anniversary turnover — copy says "tops up on the 14th" via `resetDay()`), `weights`, `memberLimit`, and two helpers every AI surface uses: **`costOf(action)`** and **`canAfford(action)`** (a photo scan costs 3, most things 1, the shopping list 0 — so aisle-sort has no guard at all). `confirmAiSpend(action)` prices the dialog by the action. A 429 with `error: "CREDIT_LIMIT"` is detected by `isCreditLimit(err)` and shown with `creditLimitMessage(err)` — the server's sentence carries the balance and reset day. Prices: £3.99/month or £29.99/year, Free 50 credits, Premium and trial 300, comps unlimited; the knobs live in the backend's `app_config` and are edited from `/back-of-house` ("Plan settings"). **The whole strategy (plans, credits, trial, seats, what each knob does) is written up in the backend repo's `MONETISATION.md` — read that before changing any pricing copy or rule.** `components/trial-prompt.tsx` is the in-app half of the trial prompts (one card per stage: four days left, last day, just ended), mounted above `PremiumBanner` in the shell; `premium-banner.tsx` is the low-credits upsell for free households.
...
| `/premium` | `(app)/premium/page.tsx` | upgrade page: monthly £3.99 / yearly £29.99 option cards (yearly badge "2 months free"), the founders' card while `GET /premium/offers` says places remain, trial-aware copy ("Keep Premium — nothing to pay until the 17th"); checkout via `authClient.subscription.upgrade({ plan: "premium", annual })` |
| `/account` | `(app)/account/page.tsx` | profile (name/email/member-since), **Premium card** (plan, credits, interval, founders' badge, billing portal), **Household** section (`household-card.tsx`: the invite form becomes an upgrade card when `GET /household` says `canInvite: false`), change password, delete account |
| `/back-of-house` | `app/back-of-house/page.tsx` (shell) + `lib/`, `components/`, `tabs/` | admin dashboard (`ADMIN_EMAILS` gate is server-side; the rail and Account show a link when `GET /shopping-list` says `isAdmin`). Tabs in the URL hash: Overview (KPIs, alerts, month-by-month from nightly snapshots + CSV), Users (tap a row for the drawer: plan, activity, recipe titles; a full recipe needs a logged reason), Engagement, Installs (installed app vs browser), Onboarding, AI & cost, Credits & revenue, Aisle cache, Settings. **Adding a panel = a file in `tabs/` + a row in `TABS`.** Data goes through `lib/use-admin-data.ts` (cached per path; 404 = empty state). Tables use `components/data-table.tsx` — one `<table>` that CSS turns into cards below 1024px via `data-label`/`data-mobile` (no duplicated markup); `layout="scroll"` for wide numeric tables. Primitives (`Kpi`, `Stat`, `Chart`, `Breakdown`, `Section`) in `components/primitives.tsx` |
...
- `components/install-gate.tsx` — the auto-open, mounted after `OnboardingGate`: once per device on phones/tablets, never on desktop, never on top of the questionnaire (if onboarding was wanted at any point this session it sets `sessionStorage fornetto:installGateSkip` and waits for the next one). Also logs `install_standalone_open` once per session — **the only true install metric on iOS**. Since the installs tab (2026-09-05) the day-to-day signal is `src/lib/client-hint.ts`: `apiFetch`/`apiSend` send `X-Fornetto-Client: <standalone|browser>/<ios|android|desktop>` on every data request and the backend marks each active day as installed-app or browser (`user_activity.standalone`, `user.installed_at`). It is self-contained (not a re-export of `install.ts`) to avoid an import cycle through `analytics.ts`.
